...
def _extract_last_available_time(variable: CopernicusMarineVariable) -> Any:
...
def _check_if_there_is_time_coordinate(
...
def _get_first_variable_with_a_time_coordinate(
    variables: list[CopernicusMarineVariable],
...
def _collect_dataset_information(
        ...
        for dataset in product.datasets:
            for version in dataset.versions:
                for part in version.parts[:MAX_PARTS_PER_VERSION]:
                    for service in _filter_allowed_services(
                        part.services, ALLOWED_SERVICES
                    ):
                        has_time = _check_if_there_is_time_coordinate(service.variables)
                        variable_name = service.variables[0].standard_name
                        last_time = None

                        if has_time:
                            variable_name, idx = (
                                _get_first_variable_with_a_time_coordinate(
                                    service.variables
                                )
                            )
                            if idx is not None:
                                last_time = _extract_last_available_time(
...
def collect_and_store_dataset_informations(
    ...
    df = _collect_dataset_information(max_products)
